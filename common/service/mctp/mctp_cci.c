...
// Section 9.16.2 indicats the CCI cmd timeout is 2s.
#define DEFAULT_WAIT_TO_MS 2000
...
mctp* mctp_cci_init(void)
{
	...
	if ((mctp_inst = mctp_init())) {
		mctp_inst->max_msg_size = MCTP_CCI_MSG_LEN_MAX;
	}
...
{
	if (msg == NULL) {
		LOG_ERR("[%s]: The pointer of cci_msg is NULL", __func__);
		return MCTP_ERROR;
	}

	if (len > MCTP_CCI_MSG_LEN_MAX) {
		LOG_ERR("[%s]: Assigned data len is invalid", __func__);
		return MCTP_ERROR;
	}

	msg->hdr.msg_payload_len = len;
...
uint8_t mctp_cci_send_msg(void *mctp_p, mctp_cci_msg *msg)
{
	...
	if (msg->hdr.msg_cate == MCTP_CCI_MSG_CATE_REQUEST) {
		...
		// Table 84. CCI Message Format indicats that
		// BO, Return Code, and Vendor must be 0 for the req msg.
		msg->hdr.bo = 0;
		msg->hdr.ret_code = 0;
		msg->hdr.vendor_state = 0;
	} else {
		LOG_ERR("Don't support this message category:%Xh", msg->hdr.msg_cate);
		return MCTP_ERROR;
	}
