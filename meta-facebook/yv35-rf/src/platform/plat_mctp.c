...
#include "mctp_cci.h"
#include "mctp_ctrl.h"
#include "plat_mctp_dev/pm8702.h"
...
{
	mctp_medium_conf pm8702_conf = { .smbus_conf.bus = PM8702_I2C_BUS,
					 .smbus_conf.addr = PM8702_I2C_ADDR };
	LOG_DBG("Init MCTP device: bus = %x, addr = %x", pm8702_conf.smbus_conf.bus,
		pm8702_conf.smbus_conf.addr);

	if (!(pm8702_cache.mctp_inst = mctp_init())) {
		LOG_ERR("mctp_init failed!!");
		return;
	}

	LOG_INF("Set mctp_inst[%p] medium config %s", (void *)pm8702_cache.mctp_inst,
	...
	// Assign TX/RX queue
	if (mctp_start(pm8702_cache.mctp_inst) == MCTP_ERROR) {
		LOG_ERR("[%s]mctp_start failure", __func__);
		return;
	}
